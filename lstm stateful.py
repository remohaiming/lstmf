# -*- coding: utf-8 -*-
"""
This is testing the following LSTM setup:
...
This setup requeires the input of a batch input shape. the data fed need to be a multiple of the batch size
I therefore added a resizing of the data accomodate this requirement.
first graph is the prediction accuracy for 1 step ahead.
secon graph show a preddiction for the whole test dataset.

this version requires tensorflow being setup for GPU as the neural network used is CuDNNLSTM. replace it by LSTM for a machine not setpu for GPU
...
########################### differences from previmodelous
resultp = []
...
#result4=np.cumsum(resubatch_input_shapelt2,axis=0)
...
model.add(CuDNNLSTM(150, return_sequences=True, batch_input_shape= (batchsi, x_train.shape[1],1), stateful=True))
...
model.add(CuDNNLSTM(75,return_sequences=True, stateful=True, batch_input_shape= (batchsi, x_train.shape[1],1)))
...
#model.add(Dropout(0.1))
model.add(CuDNNLSTM(50,return_sequences=False, stateful=True, batch_input_shape= (batchsi, x_train.shape[1],1)))
#model.add(Dropout(0.03))
#model.add(Dropout(0.1))
#model.add(Dropout(0.2))
...
"""
xt=x_test[:batchsi]
xtp=np.zeros(np.shape(xt))

predictions= model.predict(xt, batch_size=batchsi)
ys=predictions[-1]
xtp[-1,-1]=ys


for i in np.arange(batchsi, lnt):
